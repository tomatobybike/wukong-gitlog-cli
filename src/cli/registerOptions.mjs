...
export function registerOptions(program) {
  // Basic metadata is set in index.mjs (name/version/description)
  ...
  program
    ...
    .option('--limit <n>', '限制数量', (v) => parseInt(v, 10))
  ...
  program
    .option('--json', '输出 JSON')
    .addOption(new Option('--format <type>', '输出格式: text | excel | json').choices(['text', 'excel', 'json']).default('text'))
    .addOption(new Option('--group-by <type>', '按日期分组: day | month | week').choices(['day', 'month', 'week']))
  ...
  // Overtime / working hours analysis
  program
    .option('--overtime', '分析公司加班文化（输出下班时间与非工作日提交占比）')
    .addOption(new Option('--country <code>', '节假日国家：CN 或 US，默认为 CN').choices(['CN', 'US']).default('CN'))
    ...
    .option('--overnight-cutoff <hour>', '次日凌晨归并窗口（小时），默认 6', (v) => parseInt(v, 10), 6)

  // Output destination / per-period formats
  program
    ...
    .addOption(new Option('--per-period-excel-mode <mode>', 'per-period Excel 模式：sheets|files（默认：sheets）').choices(['sheets', 'files']).default('sheets'))
    .option('--per-period-only', '仅输出 per-period（month/week）文件，不输出合并的 monthly/weekly 汇总文件')

  // Serve / server
  program
    .option('--serve', '启动本地 web 服务，查看提交统计（将在 output-wukong/data 下生成数据文件）')
    .option('--port <n>', '本地 web 服务端口（默认 3000）', (v) => parseInt(v, 10), 3000)
    .option('--debug', 'enable debug logs')
    .option('--serve-only', '仅启动 web 服务，不导出或分析数据（使用 output-wukong/data 中已有的数据）')

  // Version / profiling / debug
  program
    ...
    .option('--hot-threshold <n>', 'HOT 比例阈值', (v) => parseFloat(v, 10), 0.8)
    .option('--fail-on-hot', 'HOT 时 CI 失败')
    .option('--diff-base <file>', '基线 profile.json')
    .option('--diff-threshold <n>', '回归阈值', (v) => parseFloat(v, 10), 0.2)
